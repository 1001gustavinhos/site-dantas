...
import { PhoneStepSchema, type PhoneStepFormValues } from '@/lib/schemas/checkoutSchemas';
...
export default function CheckoutPhonePage() {
  ...
  const form = useForm<PhoneStepFormValues>({
    ...
    defaultValues: {
      ...
      saveInfo: user?.phone ? !!localStorage.getItem(user.phone.replace(/\D/g, '')) : false,
    },
  });
  ...
  useEffect(() => {
    ...
    if (form.getValues('phone') !== newlyFormatted && newlyFormatted !== '') {
      form.setValue('phone', newlyFormatted, { shouldValidate: true, shouldDirty: true });
      if (isClient) {
  ...
  const handlePhoneInputChange = (event: React.ChangeEvent<HTMLInputElement>, fieldOnChange: (value: string) => void) => {
  ...
  return (
    <MainLayout>
      <section className="py-8 max-w-lg mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="font-page-title font-semibold text-2xl">{texts.contactInfoTitle}</CardTitle>
